...
echo "UCX_NET_DEVICES=mlx5_0:1 mpirun -n 2 /home/liuyao/software/Netgauge_default/netgauge D D -m mpi -x loggp -o ng_logGP_internode"

dool --time --mem --cpu --net -N ib0,ens786f1,lo,total 1 > /home/liuyao/sbatch_sh/netgauge_default/run/output/GDR/CPU.csv  &
        nvidia-smi --query-gpu=name,timestamp,uuid,utilization.gpu,memory.total,utilization.memory,power.draw --format=csv -l 1 > /home/liuyao/sbatch_sh/netgauge_default/run/output/GDR/GPU.csv &
        sh rtop.sh -d ib0 > /home/liuyao/sbatch_sh/netgauge_default/run/output/GDR/RTOP.csv  &
...
UCX_NET_DEVICES=mlx5_0:1 mpirun -n 2 /home/liuyao/software/Netgauge_default/netgauge -m mpi -x loggp -o ng_logGP_internode D D
