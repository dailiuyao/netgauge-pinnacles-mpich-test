...
# nvidia-smi --query-gpu=name,utilization.gpu,clocks.gr,clocks.mem --format=csv -l 1 > $GAUGE_OUT_DIRE/gpu_log.csv &
...
for ((itr = 0; itr < ${ITERATION_TIME}; itr += 1)); do
    ...
    #                     $MPIEXEC_HOME/bin/mpirun -n 3 --ppn 1 --cpu-bind core $NCCL_GAUGE_HOME/gauge/${mode}_gauge_n_${n}_d_${d}.exe
    #                     export GAUGE_STEP_SIZE="512"
    #                     for ((msize=${MESSAGE_SIZE_START}; msize<=${MESSAGE_SIZE_END}; msize+=${MESSAGE_SIZE_STEP})); do
