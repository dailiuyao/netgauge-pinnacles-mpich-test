...
# concurrency_sequence=(1 2 4 6 8)
...
for i in "${concurrency_sequence[@]}"; do
    for mode in broadcast; do
        ...
        nvcc "$NVCC_GENCODE" -ccbin g++ -I"${NCCL_SRC_LOCATION}/build/include" -I"${MPI_HOME}/include" \
            ...
            -D N_ITERS=${i} \
            "${NCCL_GAUGE_HOME}/gauge/${mode}_gauge.cu" -o "${NCCL_GAUGE_HOME}/gauge/${mode}_gauge_n_${i}_d_${GAUGE_D}.exe"

        # Verification of the output
        if [ -f "${NCCL_GAUGE_HOME}/gauge/${mode}_gauge_n_${i}_d_${GAUGE_D}.exe" ]; then
            echo "Compilation successful. Output file: ${NCCL_GAUGE_HOME}/gauge/${mode}_gauge_n_${i}_d_${GAUGE_D}.exe"
        else
